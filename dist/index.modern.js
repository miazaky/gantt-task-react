...
var toLocaleDateStringFactory = function toLocaleDateStringFactory(locale) {
  return function (date, dateTimeOptions) {
    var key = JSON.stringify([locale, date.getTime(), dateTimeOptions]);
...
var TaskListTableDefault = function TaskListTableDefault(_ref) {
  ...
  return React.createElement("div", {
    ...
    }, expanderSymbol), React.createElement("div", null, name))), React.createElement("div", {
      className: styles$1.taskListCell,
      style: {
        minWidth: rowWidth,
        maxWidth: rowWidth
    ...
    }))), dateTimeOptions)), React.createElement("div", {
      className: styles$1.taskListCell,
      style: {
        minWidth: rowWidth,
        maxWidth: rowWidth
...
var GridBody = function GridBody(_ref) {
  ...
  var rowCount = groupNames.length;
  var gridRows = [];
  var rowLines = [];
  ...
  for (var _i = 0; _i < dates.length; _i++) {
    ...
    var isToday = _i + 1 < dates.length && date.getTime() < now.getTime() && dates[_i + 1].getTime() >= now.getTime() || _i > 0 && _i + 1 === dates.length && date.getTime() < now.getTime() && addToDate(date, date.getTime() - dates[_i - 1].getTime(), "millisecond").getTime() >= now.getTime();
    ...
    var isTodayRTL = rtl && _i + 1 < dates.length && date.getTime() >= now.getTime() && dates[_i + 1].getTime() < now.getTime();
...
var TaskGanttContent = function TaskGanttContent(_ref) {
  ...
  return React.createElement("g", {
    ...
    fontFamily: fontFamily,
    fontSize: fontSize
  }, Object.entries(tasks.reduce(function (acc, task) {
    var _task$name;

    var key = (_task$name = task.name) != null ? _task$name : "";
    if (!acc[key]) acc[key] = [];
    acc[key].push(task);
    return acc;
  }, {})).map(function (_ref2) {
...
var Gantt = function Gantt(_ref) {
  ...
  var _useState9 = useState(ganttHeight || new Array(new Set(barTasks.map(function (t) {
  ...
  useEffect(function () {
    ...
    var rawBars = convertToBarTasks(filteredTasks, newDates, columnWidth, rowHeight, taskHeight, barCornerRadius, handleWidth, rtl, barProgressColor, barProgressSelectedColor, barBackgroundColor, barBackgroundSelectedColor, projectProgressColor, projectProgressSelectedColor, projectBackgroundColor, projectBackgroundSelectedColor, milestoneBackgroundColor, milestoneBackgroundSelectedColor);
    var rowMap = new Map();
    var nextRow = 0;
